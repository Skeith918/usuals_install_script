...
nextcloud_current_version=$(sudo -u $nextcloud_sysuser php $nextcloud_current_dir/$nextcloud_app_dir/occ --version | tail -n1 | cut -d ' ' -f3)
...
unzip $nextcloud_current_dir/nextcloud-$nextcloud_wanted_version.zip -d $nextcloud_current_dir/.
...
chown -R $nextcloud_sysuser:www-data $nextcloud_current_dir/$nextcloud_app_dir
